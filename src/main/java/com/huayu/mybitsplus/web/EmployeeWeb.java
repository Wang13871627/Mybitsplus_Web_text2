...
import com.huayu.mybitsplus.pojo.*;
...
public class EmployeeWeb {
    static Logger logger=Logger.getLogger(EmployeeWeb.class);
    @Autowired
    ...
    @Autowired
    private IService<PostAuth> postAuthIService;
    @Autowired
    private AuthorityUtils authorityIService;
    ...
    public void add2(Employee employee) {
        ...
        Object salt = ByteSource.Util.bytes(employee.getName());
        /*加密次数*/
        Integer hashIterations = 1024;
        /*盐值加密username*/
        Object s = new SimpleHash(hashAlgorithmName, pwd, salt, hashIterations);
        employee.setPassword(s.toString());
        employeeService.save(employee);
    }
    ...
    public String login(String name, String password, Model model) {
        ...
        try {
            ...
            i=1;
            model.addAttribute("login","用户不存在");
        } catch (IncorrectCredentialsException e) {
            i=2;
            model.addAttribute("login","密码输入错误");
        }
        if(i==1){
            return "redirect:/layulview/login.html";
        }else if(i==2){
            return "redirect:/layulview/login.html";
        }else{
            return "redirect:/layulview/frame.html";
        }
    }

    @RequestMapping("/auth.do")
    @ResponseBody
    @RequiresRoles(value = {"财务统计"})
    public EmpUtils tree1(Integer page,Integer limit) {
        EmpUtils empUtils = new EmpUtils();
        Page<Authority> page1=new Page<>(page,limit);
        List<Authority> list =authorityIService.queryall(page1);
        for(Authority authority:list){
            if(authority.getStatus().equals("1")){
                authority.setStatus("可用");
            }else{
                authority.setStatus("禁用");
            }
        }
        empUtils.setCode(0);
        empUtils.setMsg("查询成功");
    ...
    @ResponseBody
    @RequiresRoles(value = {"财务统计"})
    public EmpUtils post(int page, int limit) {
        Page<Position> page1 = new Page<>(page, limit);
        List<Position> list =positionService.queryall(page1);
        EmpUtils empUtils = new EmpUtils();
        empUtils.setCode(0);
        empUtils.setMsg("查询成功");
        empUtils.setCount(Integer.valueOf(String.valueOf(page1.getTotal())));
    ...
    * 权限管理
    * */
    @RequestMapping("/autha.do")
    @ResponseBody
    public void autha(Authority authority) {
        authority.setStatus("1");
        authorityIService.save(authority);
    }

    @RequestMapping("/authd.do")
    @ResponseBody
    public PicUtils authd(Integer aid) {
        PicUtils picUtils = new PicUtils();
        if (authorityIService.removeById(aid)){
            picUtils.setCode("1");
            picUtils.setMsg("删除成功");
        }else{
            picUtils.setCode("0");
    ...
    @RequestMapping("/authd1.do")
    @ResponseBody
    public PicUtils authd1(String eids) {
        PicUtils picUtils = new PicUtils();
        try {
            String[] str = eids.split(",");
            for (int i = 0; i < str.length; i++) {
                authorityIService.removeById(str[i]);
            }
            picUtils.setCode("1");
            picUtils.setMsg("删除成功");
        } catch (Exception e) {
    ...
    @RequestMapping("/authu.do")
    public String authu(Integer aid,Model model) {
        Authority authority=authorityIService.getById(aid);
        model.addAttribute("auth",authority);
        return "/layulview/authu.html";
    }

    @RequestMapping("/authu1.do")
    @ResponseBody
    public PicUtils authu1(Authority authority) {
        PicUtils picUtils = new PicUtils();
        if (authorityIService.updateById(authority)){
            picUtils.setCode("1");
            picUtils.setMsg("修改成功");
        }else{
            picUtils.setCode("0");
            picUtils.setMsg("修改失败");
        }
        return picUtils;
    }
    ...
    @ResponseBody
    public PicUtils posta(Position position) {
        PicUtils picUtils=new PicUtils();
        if(positionService.save(position)){
            picUtils.setCode("1");
            picUtils.setMsg("添加成功");
        }else{
            picUtils.setCode("0");
            picUtils.setMsg("添加失败");
        }
        String str=null;
        try {
            str.equals("");
    ...
    @RequestMapping("/postd1.do")
    @ResponseBody
    public PicUtils postd1(String pid) {
        PicUtils picUtils = new PicUtils();
        try {
            String[] str = pid.split(",");
            for (int i = 0; i < str.length; i++) {
                positionService.removeById(str[i]);
            }
            picUtils.setCode("1");
            picUtils.setMsg("删除成功");
        } catch (Exception e) {
    ...
    @RequestMapping("/postd.do")
    @ResponseBody
    public PicUtils postd(Integer pid) {
        PicUtils picUtils = new PicUtils();
    ...
    @RequestMapping("/postu.do")
    public String postu(Integer pid,Model model) {
        Position position=positionService.getById(pid);
        model.addAttribute("post",position);
        return "/layulview/postu.html";
    }

    @RequestMapping("/postu1.do")
    @ResponseBody
    public PicUtils postu1(Position position) {
        PicUtils picUtils = new PicUtils();
    ...
    @RequestMapping("/postauth.do")
    public String postauth(Integer pid,Model model) {
        Position position=positionService.getById(pid);
        List<Authority> list=authorityIService.list();
        QueryWrapper queryWrapper=new QueryWrapper();
        queryWrapper.eq("pid",pid);
        List<PostAuth> list1=postAuthIService.list(queryWrapper);
        StringBuffer buffer=new StringBuffer();
        if(list1.size()>0){
            for(PostAuth postAuth:list1){
                Authority authority=authorityIService.getById(postAuth.getAid());
                buffer.append(authority.getAname()).append(",");
            }
        }
        String str="";
        if(buffer.length()>0){
        ...
        model.addAttribute("post",position);
        model.addAttribute("list",list);
        return "/layulview/postauth.html";
    }

    @RequestMapping("/postauth1.do")
    @ResponseBody
    public PicUtils postauth1(Integer pid,String aid,Model model) {
        PicUtils picUtils = new PicUtils();
        try {
            QueryWrapper queryWrapper=new QueryWrapper();
            queryWrapper.eq("pid",pid);
            List<PostAuth> list=postAuthIService.list(queryWrapper);
            String[] str = aid.split(",");
            for (int i = 0; i < str.length; i++) {
                if(list.size()>0){
                    for (PostAuth postAuth:list){
                        if(!str[i].equals(postAuth.getAid())){
                            postAuthIService.removeById(postAuth.getId());
                        }
                    }
                }else{
                    PostAuth postAuth=new PostAuth();
                    postAuth.setPid(pid);
                    postAuth.setAid(Integer.valueOf(str[i]));
                    postAuthIService.save(postAuth);
                }
            }
            picUtils.setCode("1");
            picUtils.setMsg("删除成功");
